...
| **`claim_ready` stays empty *forever* after the Wan fp8 download finishes; needs a manual ComfyUI/agent restart** | The agent inventoried models at startup (before the 74GB `wan2.2_*_14B_fp8_scaled` download completed) and ComfyUI caches its `diffusion_models` folder listing at boot — so `/object_info/UNETLoader` never reflects the late-arriving weights and `claim_ready=wan` never populates. | Fixed in repo (`agent/loboforge_agent_eventforge.py` + `loboforge_agent_common.py`): `ef_claim_ready_reconcile_loop` re-inventories every ~45s and, when Wan weights are on disk but the UNET list is stale (`comfy_inventory_stale_vs_disk`), force-restarts ComfyUI (idle-only, rate-limited) so it re-indexes and `claim_ready` recovers on its own. Requires fleet patch/deploy of the agent scripts. |
| **New/recently-rented box quarantined or churned as "not ready / instance gone" while still provisioning** | Monitors/reapers act on the fleet snapshot: a healthy box mid-download reports empty `claim_ready`, which produced reap-worthy `no-claim-ready` / `wan-not-ready` / `idle-no-jobs` badges immediately. | Fixed in repo: EventForge now tracks `first_seen_at` and a `provisioning_grace` window (default 30 min, `EVENTFORGE_WORKER_PROVISION_GRACE_MINUTES`). During grace, a healthy no-models box emits a benign `provisioning` badge instead of the not-ready badges, and `/v1/ops/fleet` + `/v1/fleet/workers` expose `first_seen_at`, `age_seconds`, `provisioning_grace`. Monitors (e.g. `archon-fleet-monitor`) must skip boxes with `provisioning=true` / `provisioning_grace=true`. |
| **LoRA-referencing jobs flood claim→release→fail; missing files never on disk** (e.g. `wan2.2_spread_*`, `wan2.2_pt_miss_smashcut_copy_*`) | The referenced LoRA is not published anywhere a worker can fetch it: not in the EventForge asset library (`GET /v1/jobs/{id}/loras/{file}`), not in LoboForge `active-loras`/`tool-loras`. Workers correctly refuse to run without the file: they release (then terminal-fail after `FORGE_QUEUE_MAX_LORA_DEFERS`, default 15). | **Not a worker/agent bug — the LoRA must be uploaded.** Publish the missing LoRA to the EventForge asset library via `POST /v1/assets/loras` (preferred) or add it to LoboForge `active-loras` for the box mode. Until then these jobs will keep churning. Do **not** fabricate placeholder LoRA files. |
| **Worker says `claim_ready=wan` but remains idle while wan jobs are queued** | EventForge's model/LoRA gate found no job the worker could run. Historically `/v1/jobs/claim` returned an indistinguishable 204, so the agent looked healthy and retried forever. This commonly followed a failed/partial active-LoRA sync or stale Comfy inventory. | Fixed in repo: empty claim responses include bounded `X-EventForge-Queued-*` / `Blocked-*` diagnostics. Idle workers reconcile every 5 minutes (and immediately after becoming idle): they query `GET /v1/workers/loras/needed` for ready EventForge assets referenced by queued jobs, download and structurally validate them, remove corrupt copies, refresh `known_loras`, and retry failures every minute; the independent LoboForge active-LoRA sync still runs if EventForge prefetch fails. **Readiness contract:** `claim_ready=wan` requires the complete validated Wan stack on disk (high+low UNETs, umt5 text encoder, VAE); `known_loras` only lists validated files; `BuildLoraGate` requires each job LoRA in that validated worker inventory. A ready server asset alone is no longer sufficient to lease a job. Vanilla no-LoRA jobs remain eligible throughout. Deploy EventForge, then patch existing workers so both protocol sides are current. |

### Missing customer LoRAs (`wan2.2_spread_*`, `wan2.2_pt_miss_smashcut_copy_*`, …)

If the queue shows a repeating **claim → release → fail** loop for jobs that reference specific LoRA basenames, the worker is doing the right thing — it never runs a job whose LoRA is not on disk. The agent tries, in order: the per-job EventForge LoRA endpoint, LoboForge `active-loras`/`tool-loras`, then a bulk mode sync; after `FORGE_QUEUE_MAX_LORA_DEFERS` (default 15) attempts it terminal-fails with `missing LoRAs after N attempts: …`.

The fix is to make the file fetchable, not to change the worker:

- **Preferred:** upload the LoRA to the EventForge asset library so any worker can pull it per-job:
  `POST /v1/assets/loras` (app API key) → workers download with `GET /v1/jobs/{id}/loras/{file}`.
- **Fallback:** register it in LoboForge `active-loras` (with a working `source_url`) for the relevant mode (`video`/`image`/`all`).

These specific names (`wan2.2_spread_*`, `wan2.2_pt_miss_smashcut_copy_*`) are **not** present in any repo catalog and must be supplied by the integrator; do not invent stand-in `.safetensors`.
