...
await sqliteS3.RestoreOnStartupAsync();

var persist = app.Services.GetRequiredService<WriteBehindPersistence>();
await persist.LoadAsync(CancellationToken.None);
...
static async Task RunPingLoopAsync(WsSession session, TimeSpan interval, CancellationToken ct)
{
