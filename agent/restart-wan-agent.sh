#!/bin/bash
# Surgical restart of the EventForge WAN (Comfy) agent supervisor loop on a Vast box.
# Recreates the `loboforge-agent` tmux session with the same while-true supervisor
...
#
# Served at /agent/restart-wan-agent.sh and baked into the image so the on-box
# watchdog (wan-supervisor-watchdog.sh) can (re)fetch and invoke it after the
# whole tmux supervisor dies. Mirror of scripts/restart-wan-agent.sh.
set -euo pipefail
